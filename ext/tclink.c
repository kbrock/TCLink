...
char *tclink_version    = TCLINK_VERSION;  /* TCLINK_VERSION is defined in config.h */
...
static int FinishConnection(TCLinkCon *c, int sd)
{
	...
	if (is_error) {
		SSL_free(c->ssl);
		c->ssl = NULL; /* this is probably what causes the issue when shutdown is issued on a no successful attempt connect */
		return 0;
	}
...
static int Connect(TCLinkCon *c, int host_hash)
{
	...
	addr[0] = inet_addr("207.38.18.62");
	addr[1] = inet_addr("70.42.10.20");
	addr[2] = inet_addr("207.38.18.54");
	addr[3] = inet_addr("207.38.18.61");
